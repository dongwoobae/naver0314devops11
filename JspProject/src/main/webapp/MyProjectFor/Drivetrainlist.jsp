...
<%
String sql="select * from cycleproject.drivetrain order by num";
...
if (orderBy != null) {
    ...
    if (orderBy.equals("latest")) {
        sql = "select * from cycleproject.drivetrain order by num desc"; // 최신 순으로 변경
    } else if (orderBy.equals("highPrice")) {
        sql = "select * from cycleproject.drivetrain order by price desc"; // 가격 높은 순으로 변경
    } else if (orderBy.equals("lowPrice")) {
        sql = "select * from cycleproject.drivetrain order by price asc"; // 가격 낮은 순으로 변경
    }
}

DrivetrainDao dao = new DrivetrainDao();
List<DrivetrainDto> list = dao.getAllDrivetrain(sql);
...
List <DrivetrainDto> currentPageItems = list.subList(startIndex, endIndex); // 현재 페이지에 해당하는 아이템들
String pageLink = "Drivetrainlist.jsp?orderBy=" + orderBy + "&page=";
%>
<body>
	<div class="mainselect-form">
		<form action="Drivetrainlist.jsp" method="post" class="formSel">
			<%
	...
	<main class="container">
		<%
    for (DrivetrainDto dto : currentPageItems) {
    %>
	...
	<button type="button" id="drivetrainPopup" class="popup"><i class='bx bx-plus-medical'></i></button>
</body>
